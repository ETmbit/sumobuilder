...
namespace PxDistance {
    ...
    export class Device {
        ...
        read(): number {
            ...
            /*
            include with mixed usage of micro:bit version 1 and higher
...
namespace SumoBuilder {
    ...
    function notrace(): number {
        return (ETsensorDist.read() > fielddiameter ? 1 : 0)
    }
    ...
    function far(): number {
        let cm = ETsensorDist.read()
        return (cm > neardistance && cm <= fielddiameter ? 1 : 0)
    }

    function near(): number {
        return (ETsensorDist.read() <= neardistance ? 1 : 0)
    }
